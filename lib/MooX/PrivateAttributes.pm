...
sub import {
    ...
    return if $target->can('private_has') && $target->can('private_with_deprecated_has');	
    ...
    if ( my $info = $Role::Tiny::INFO{$target} ) {
        ...
        $info->{not_methods}{$private_with_deprecated_has} = $private_with_deprecated_has;
    }

    { no strict 'refs'; *{"${target}::private_has"} = $private_has }
    { no strict 'refs'; *{"${target}::private_with_deprecated_has"} = $private_with_deprecated_has }
